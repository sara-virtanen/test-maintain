...
// Integration tests for Hex-to-RGB API.

/**
 * @module test/integration
 * @description
 * Integration tests for the Hex-to-RGB REST API.
 * Uses Supertest to simulate HTTP requests and Chai for assertions.
 */
...
describe('Integration Tests: Hex-to-RGB API', () => {
    /**
     * Tests that a valid hex code returns status 200 and correct JSON body.
     */
    ...
    /**
     * Tests that a lowercase hex code is handled correctly.
     */
    ...
    /**
     * Tests that boundary values (black and white) are handled correctly.
     */
    ...
    /**
     * Tests that an invalid hex code returns status 400 and an error message.
     */
    ...
    /**
     * Tests that a hex code with invalid characters returns status 400.
     */
    ...
    /**
     * Tests that a too-short or too-long hex code returns status 400.
     */
    ...
    /**
     * Tests that a completely missing route returns 404.
     */
    it('returns 404 for missing route', async () => {
        const res = await request(app).get('/hex-to-rgb/');
        expect(res.status).to.be.oneOf([404, 400]); // depending on how Express handles it
    });
});
